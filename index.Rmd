...
Defense_pie <- reactive({
  ...
  mydata <- EI_industry %>% 
    ...
    filter(Attribute %in% attribute_selection) %>% 
    rename(imp = !!impact_selection) %>% 
    rename(att = !!attribute_selection) %>% 
    mutate(
      lab = case_when(
        impact_selection == 'Employment' ~ paste(prettyNum(round(imp, digits = 0), big.mark = ","), "jobs"), 
        T ~ paste("$", prettyNum(round(imp, digits = 0), big.mark = ","))))
...
mainPanel(
  column(6,
         selectInput('impact_selection', 'Select economic impact:', selected = "Output", choices = list("Output" = "Output", "Employment" = "Employment","Income" = "Personal.Income"))),
  column(6,
         selectInput('attribute_selection', 'Select attribute:', selected = "Direct Industry Breakdown", choices = c("Direct Industry Breakdown" = "Direct.Industry.Breakdown", "Type of Contribution" = "Type.of.Contribution"))))
```
<br>
<br>

```{r}
output$Defense_pie <- renderPlotly(Defense_pie())
plotlyOutput("Defense_pie")
```
...
Living.Resources_OutputIndustry <- EI_industry %>%
  ...
  plot_ly(
    width = 800,
    height = 350,
    labels = ~Direct.Industry.Breakdown,
    values = ~Output,
    type = "pie",
    sort = FALSE,
    direction = "clockwise",
