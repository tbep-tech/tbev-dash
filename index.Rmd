...
ES_carbon2 <- read.csv("data/ES_carbon2.csv")
...
carbon_bar <- reactive({
  ...
  CarbonQuantity <- mydata %>%
    ...
    text = ~LabelQ) %>%
  layout(
    xaxis = list(title = "Quantity (tonnes CO2/yr)"),
    yaxis = list(title = ""),
    hoverlabel = list(
      font = list(family = "Arial", size = 16, color = "white")))
  CarbonValue <- mydata %>%
...
mainPanel(
  column(6,
         selectInput('flow_selection', 'Select flow:', selected = "Sequestration", choices = unique(ES_carbon2$Flow))),
  column(6,
         selectInput('scale_c_selection', 'Select scale:', selected = "Per acre", choices = unique(ES_carbon2$Scale))))
```
<br>
<br>
<br>
